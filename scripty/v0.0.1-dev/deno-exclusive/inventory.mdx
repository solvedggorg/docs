...
| `Deno.resolveDns` | `exclusive` | Deno DNS API | No first-class Node/Bun equivalent shape |
...
| Snapshot testing APIs | `exclusive` | Deno test helpers | Confirm pin |
...
| API | Kind | Notes |
| --- | --- | --- |
| `Deno.cron` (if still runtime API in pin) | `exclusive` | Confirm pin |
| OpenTelemetry zero-config hooks unique to Deno | `exclusive` | Observability; may be host |
| `Deno.cwd` permission interactions | `shape-only` | Tie to permissions |
| Lint plugin runtime APIs | `exclusive` | Only if stable runtime surface |
