...
| `Bun.serve` | Signature HTTP server API (fetch handler style) | Deno has `Deno.serve` (different initiative); Node needs `http`/`node:http` |
...
| `Bun.open` (editor helper) / related DX | Bun-only DX | Optional low priority |
...
| `Bun.SHA*` convenience | Bun helpers | May collapse into hash API |
