...
```bash
rusty init [name] [-cli|-web|-tui|-lib|…] [--format cargo|rusty] [--edition YYYY] [-y]
```
...
| `--cli` / `-cli`, `--web` / `-web`, `--tui` / `-tui`, `--lib` / `-lib`, `--fullstack` / `-fullstack`, `--blank` / `-blank` | Template kind (exactly one; same as `--kind`) |
