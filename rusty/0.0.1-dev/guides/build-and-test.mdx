...
Native drivers (no cargo):

```bash
rusty clippy [-p PKG] [--workspace]   # needs clippy component
rusty fmt [--check] [-p PKG] …        # needs rustfmt component
```

Install components via the toolchain manager:
...
Lifecycle aliases prefer scripts when present, otherwise native:

```bash
rusty lint      # scripts.lint, else native clippy
rusty format    # scripts.format, else native fmt
```

Cargo interop remains only for **`doc` / `bench` / `tree`** (need cargo on PATH). Prefer scripts for monorepo policy when you need custom flags.
