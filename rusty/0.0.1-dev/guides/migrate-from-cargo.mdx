...
Native clippy/fmt need components (not cargo). Interop-only verbs still expect cargo on PATH:
...
rusty env install stable cargo   # only if you need doc/bench/tree
rusty doc
```
