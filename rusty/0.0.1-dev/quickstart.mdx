---
title: "Quickstart"
description: "From zero to a working rusty project on Linux: toolchain, init, lock, fetch, build, test."
---

Get a minimal Rust project building with rusty on Linux.

## Prerequisites

- A **Linux** host (x86_64 or aarch64 typical)
- Network access for the first toolchain install and crate fetch
- A linker (`cc` / `ld`) available for linking Rust binaries

## 1. Install rusty

Obtain a `rusty` binary for your machine and put it on `PATH`. See [Install](/rusty/0.0.1-dev/install) for current distribution options.

```bash
rusty --version
# rusty 0.0.1-dev (version string may differ)
```

Data lives under **`$RUSTY_HOME`** (default `~/.rusty`).

## 2. Install a toolchain

Default installs include **rustc + rust-std only** (no cargo):

```bash
rusty env install stable
rusty env use stable
rusty env show
```

Optional: put toolchain shims on your path after `rusty env use`:

```bash
export PATH="$RUSTY_HOME/bin:$PATH"
# default RUSTY_HOME is ~/.rusty
export PATH="${RUSTY_HOME:-$HOME/.rusty}/bin:$PATH"
```

`rustc`, `rustdoc`, `rustfmt`, and related tools are hardlinked shims that dispatch to the active toolchain. **cargo is not installed or shimmed by default.**

## 3. Create a project

On a TTY, `rusty init` prompts for unset fields. For scripts and CI, pass flags so nothing is interactive:

```bash
mkdir hello-rusty && cd hello-rusty
# name + kind short-flag + non-interactive
rusty init hello-rusty --cli -y --format rusty --edition 2021

# equivalent forms
rusty init lib -cli
rusty init myapp --web -y --format rusty
rusty init api --kind fullstack --edition 2024 -y
```

Useful flags:

| Flag | Meaning |
| --- | --- |
| `--format rusty` | Scaffold `rusty.json` + `rusty.lock` (native-first) |
| `--format cargo` | Scaffold `Cargo.toml` for interop / gradual migration |
| `--kind KIND` | `web` \| `cli` \| `tui` \| `lib` \| `fullstack` \| `blank` |
| `--cli` / `-cli`, `--web` / `-web`, … | Same as `--kind` (exactly one kind) |
| `--edition 2021` | Rust edition (`2015` \| `2018` \| `2021` \| `2024`) |
| `--yes` / `-y` | Non-interactive even on a TTY (defaults for unset fields) |

Interactive menu default kind is **cli**; non-interactive default kind is **blank**. See `rusty init --help`.

Project metadata and caches land in **`.rusty/`** (including `.rusty/format`).

## 4. Lock and fetch

```bash
rusty lock
rusty fetch
```

- **Lock** writes `rusty.lock` or `Cargo.lock` depending on project format.
- **Fetch** resolves (if needed) and ingests crates into the content-addressable store **in parallel** (worker count capped like sparse-index prefetch; honors `RUSTY_JOBS` / `-j` budget). When the lockfile already pins a checksum, rusty builds the crates.io static download URL and skips a sparse-index round-trip.

## 5. Build and test

```bash
rusty build
rusty test
```

Native-only for these verbs: if the project shape is outside the supported matrix, rusty **exits with an error** instead of silently calling cargo.

Optional package and workspace flags (same spirit as Cargo):

```bash
rusty build -p mypkg --workspace
rusty build --affected main
rusty test -j 8
```

## 6. Day-to-day commands

```bash
rusty add serde --vers "^1"
rusty rm some_crate
rusty update
rusty clean          # removes <project>/.rusty/target
rusty script <name>  # run a named entry from rusty.json "scripts"
```

Lint and format require scripts in `rusty.json`:

```json
{
  "scripts": {
    "lint": "…",
    "format": "…"
  }
}
```

```bash
rusty lint
rusty format
```

## Existing Cargo workspace?

You do not have to rewrite everything on day one:

1. Point rusty at the tree (detects `Cargo.toml` / cargo-format).
2. Use native lock/fetch/add where supported.
3. When ready: `rusty migrate` to convert to `rusty.json` + `rusty.lock`.

See [Migrate from Cargo](/rusty/0.0.1-dev/guides/migrate-from-cargo).

## Next

- [Project formats](/rusty/0.0.1-dev/concepts/project-formats) — rusty vs cargo layout
- [Build and test](/rusty/0.0.1-dev/guides/build-and-test) — flags, affected builds, scripts
- [CLI reference](/rusty/0.0.1-dev/reference/cli) — full command map
