...
manifest (rusty.json or Cargo.toml)
  ...
  → fetch (lock pin → static URL + checksum; else index + checksum)
  → parallel CAS ingest / materialize
...
- **`clippy` / `fmt`**: native drivers (`clippy-driver` / `rustfmt`); install components with `rusty env install <id> clippy` / `rustfmt`.
- **Cargo interop verbs**: tiny allowlist (`doc`, `bench`, `tree`); need cargo on PATH.
