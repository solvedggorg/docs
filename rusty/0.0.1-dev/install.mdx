...
Without cargo on PATH, interop commands (`doc`, `bench`, `tree`) fail closed (exit **127**). Native `clippy` / `fmt` need the matching components (`rusty env install <id> clippy` / `rustfmt`), not cargo.
